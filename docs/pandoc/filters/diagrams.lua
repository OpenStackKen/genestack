...
logging = false
logMessage("INFO","In Diagrams filter...")
...
--- Convert metadata-provided command values into the shapes `pipe()` expects.
--
-- Pandoc metadata can surface sequences as MetaList values, while environment
-- variables and hard-coded defaults arrive as strings. Normalizing here keeps
-- the engine compile paths simple and lets the defaults file describe
-- multi-part commands such as `docker run ...`.
local function normalize_command (command)
  if not command then
    return nil
  end

  local value_type = pandoc.utils.type(command)
  if value_type == 'List' then
    return command:map(stringify)
  elseif value_type == 'MetaList' then
    local normalized = List{}
    for _, item in ipairs(command) do
      normalized:insert(stringify(item))
    end
    return normalized
  end
...
local function get_engine (name, engopts, format)
  ...
  local execpath = normalize_command(
    engopts.execpath or os.getenv(name:upper() .. '_BIN')
  )
