---
title: "Freezer Backup Retention Policy"
---

## Overview

The `swift_retention_policy.py` script helps you automatically set object
expiration and clean up empty Swift containers after all objects have expired
and been deleted.

> [!NOTE]
>
> This script has only been tested with Freezer backup containers created in
> Swift, but it should work with any Swift containers and objects.
...
> [!TIP]
>
> Always validate with a short expiration before applying to production
> containers.
...
>
> Problem: the container shows as empty but will not delete.
>
> Solutions:
...
>
> Problem: objects have `X-Delete-At` set but are not being deleted.
>
> Solution: verify the `swift-object-expirer` daemon is running and healthy on
> the Swift cluster.
...
> Script times out
>
> Problem: the script reaches `max-wait` before the container is empty.
>
