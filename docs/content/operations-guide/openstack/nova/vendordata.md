...
The tracked plumbing for this lives in `/opt/genestack/base-kustomize/nova/base/static-vendordata-configmap.yaml`.
In deployed environments this is typically overridden from `/etc/genestack/kustomize/nova/base/static-vendordata-configmap.yaml` and then consumed by the Nova metadata service at `/etc/nova/vendor_data.json`.
