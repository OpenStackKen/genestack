...
[MariaDB](https://mariadb.org/) or [MySQL](https://dev.mysql.com/community/) are the standard databases for OpenStack.  To have redundancy, [Galera](https://galeracluster.com/) clustering can be used to provide multi-read/write capabilities. Regular database backups should be maintained and transferred to multiple locations outside the cluster for emergency cases.

Message queues  should be configured in a distributed mode for redundancy. The most common message queue used for OpenStack is [RabbitMQ](https://www.rabbitmq.com/), which has [various](https://www.rabbitmq.com/docs/reliability) capabilities that can be implemented to provide reliability and redundancy.

In some cases, with larger deployments, services might have to be separated and deployed in their own dedicated infrastructures.  The OpenStack [Large Scale SIG](https://docs.openstack.org/large-scale/index.html) provides documentation on various scaling techniques for the various OpenStack services, as well as guidance around when it is appropriate to isolate a service or to scale it independently of other services.
...
To effectively manage compute node redundancy, subdivide your nodes into multiple [Host Aggregates (HAs)](/design-guide/cloud-design/host-aggregates/) and assign one or more spare compute nodes with the same capabilities and resources to each aggregate. These spare nodes must be kept free of load to ensure they can accommodate instances from a failed compute node. WHen you are creating [Availability Zones (AZs)](/design-guide/cloud-design/availability-zones/) from host aggregates, you allow users to select where their instances are deployed based on their requirements. If a Compute Node fails within an AZ, the instances can be seamlessly evacuated to the spare node(s) within the same AZ. This minimizes disruptions and maintains service continuity.
