...
Make sure to update the `octavia-preconf-main.yaml` with the correct region, auth url, and password.
...
Use these steps on Debian/Ubuntu for creating a Python virtualenv for running the Octavia pre-deployment playbook:
...
> [!example]
>
> Running the playbook can be fully dynamic by using the following example...
