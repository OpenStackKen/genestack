...
```mermaid
flowchart TD
      fail(Compute Host Fails)
      detect(<code>masakari-hostmonitor</code> Detects Failure)
      notify(Notification Sent to <code>masakari-api</code>)
      process(<code>masakari-engine</code> Processes Failure)
      remediate(Nova Migrates VMs to Another Host)

fail --> detect --> notify --> process --> remediate
```

> [!note]
>
> Masaakari can use the **Kubernetes Driver** when OpenStack runs on Kubernetes.  The monitor queries the Kubernetes API to check node status.
